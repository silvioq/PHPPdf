...
class StylesheetParser extends XmlParser
{
    ...
    public function addConstraintsFromAttributes(BagContainer $constraint, \XMLReader $reader, array $ignoredAttributes = [self::ATTRIBUTE_CLASS])
    {
        while ($reader->moveToNextAttribute()) {
            ...
            foreach ($attributes as $name => $value) {
                if (!in_array($name, $ignoredAttributes)) {
                    if ($complexAttributeName = $this->getComplexAttributeName($name)) {
                        ...
                        $constraint->add($complexAttributeName, ['name' => $complexAttributeName, $propertyName => $value]);
                    } else {
    ...
    private function getComplexAttributeName($name)
    {
        ...
        foreach ($complexAttributesNames as $complexAttributeName) {
            if (strpos($name, $complexAttributeName) === 0 && in_array(substr($name, strlen($complexAttributeName), 1), ['.', '-'])) {
